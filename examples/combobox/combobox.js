import { debounce, getReactProp } from '../../lib';
import css from './combobox.css?inline';
...
export default {
  ...
  constructorCallback() { 
    ...
    const inputListener = srcFunc ? 
      debounce(() => srcFunc(inputEl.value).then((resp) => {
   ...
   */
  rewriteListEl(listEl, rows, template) {
   ...
   */
  scrollIfNeeded(container, element) {
